...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (
            ...
            $supportsInlineForeignKeys = ! $isSqlite;
            ...
            Schema::table('orders', function (Blueprint $table) use (
                ...
                if ($missingCustomerId) {
                    $customerColumn = $table->unsignedBigInteger('customer_id')->nullable();

                    if (! $isSqlite) {
                        $customerColumn->after('id');
                    }
                ...
                if ($missingProductId) {
                    $productColumn = $table->unsignedBigInteger('product_id')->nullable();

                    if (! $isSqlite) {
                        $productColumn->after('customer_id');
                    }
                ...
                if ($missingQuantity) {
                    $quantityColumn = $table->unsignedInteger('quantity')->default(1);

                    if (! $isSqlite) {
                        $quantityColumn->after('product_id');
                    }
                }
                ...
                if ($missingStatus) {
                    ...
                    if (! $isSqlite) {
                        $statusColumn->after('quantity');
                    }
                }

                if ($missingOrderDate) {
                    $orderDateColumn = $table->date('order_date')->nullable();

                    if (! $isSqlite) {
                        $orderDateColumn->after('status');
                    }
                }

                if ($missingDeliveryDate) {
                    $deliveryDateColumn = $table->date('delivery_date')->nullable();

                    if (! $isSqlite) {
                        $deliveryDateColumn->after('order_date');
                    }
                }
                ...
                if ($missingNotes) {
                    ...
                    if (! $isSqlite) {
                        $notesColumn->after('delivery_date');
                    }
                }
            });
        }
